...
clients_models = []
accuracies = []
top_k_prime = 3  # 從已訓練的 K 個 client 中選擇準確率最高的 top K' 進行更新
...
for round_num in range(n_rounds):
    ...
    selected_clients = random.sample(range(n_clients), k)  # 從 N 個 client 中隨機選擇 K 個進行訓練
    selected_clients_data_sizes = [len(clients_datasets[i]) for i in selected_clients]

    # 儲存每個 client 在 n_iterations 次訓練中的模型和平均準確率
    client_avg_accuracies = []
    client_avg_models = []

    # 對每個選出的 client 進行多次訓練
    for i in selected_clients:
        ...
        for iteration in range(n_iterations):
            print(f"Iteration {iteration + 1} for Client {i + 1}")
            # 初始化 client model 並載入 global model 的權重
        ...
        # 計算 client 多次迭代的平均準確率
        avg_accuracy = sum(iteration_accuracies) / len(iteration_accuracies)
        client_avg_accuracies.append(avg_accuracy)
        client_avg_models.append(iteration_models[0])  # 使用最後一次的模型
        print(f'Average Accuracy for Client {i + 1}: {avg_accuracy:.2f}%')

    # 在已選擇的 K 個客戶端中，根據平均準確率選擇 top K' 進行更新
    top_clients_indices = sorted(range(len(client_avg_accuracies)), key=lambda idx: client_avg_accuracies[idx], reverse=True)[:top_k_prime]
    top_clients_models = [client_avg_models[idx] for idx in top_clients_indices]
    top_clients_data_sizes = [selected_clients_data_sizes[idx] for idx in top_clients_indices]

    # Federated Averaging：只對 top K' 客戶端進行模型聚合
    global_state_dict = global_model.state_dict()
    total_data_size = sum(top_clients_data_sizes)

    for key in global_state_dict.keys():
        global_state_dict[key] = sum(
            (top_clients_data_sizes[i] / total_data_size) * top_clients_models[i].state_dict()[key]
            for i in range(top_k_prime)
        )
